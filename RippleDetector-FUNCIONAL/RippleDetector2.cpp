                            ...
                            MidiBuffer& events) //This is the core of the code, is the script that will run when every buffer comes
{
 ...
 /*THRESHOLD*/  ThresholdAmplitude = module.MED + 2.00*sqrt(module.STD/(module.AvgCount*4)); //building the threshold from average + n*standard deviation                
                
                if ( sample >=  ThresholdAmplitude & RefratTime > 2 ) //counting how many points are above the threshold and if has been 2 s after the last event (refractory period)
                {                
                  module.count++;
                }
		              else if(sample < ThresholdAmplitude & i == 0)//protect from acumulation
		            {
		                 module.count = 0;
 	             	}               
