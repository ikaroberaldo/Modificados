...
ThresholdNode::ThresholdNode()
    : GenericProcessor  ("Threshold Inputs")                                                                  //Adiciona Título
    , defaultLowCut     (0.0f)                                                                                //Adiciona Valor do Low
    , defaultHighCut    (0.0f)                                                                                //Adiciona Valor do High
{
...
AudioProcessorEditor* ThresholdNode::createEditor()
{
    editor = new ThresholdEditor (this, true);

    ThresholdEditor* ed = (ThresholdEditor*) getEditor();
...
void ThresholdNode::updateSettings()
{
    //int id = nodeId;
...
double ThresholdNode::getLowCutValueForChannel (int chan) const
{
    return lowCuts[chan];
}


double ThresholdNode::getHighCutValueForChannel (int chan) const
{
    return highCuts[chan];
}


bool ThresholdNode::getBypassStatusForChannel (int chan) const
{
    return shouldFilterChannel[chan];
}


void ThresholdNode::setFilterParameters (double lowCut, double highCut, int chan)
{
...
void ThresholdNode::setParameter (int parameterIndex, float newValue)
{
...
void ThresholdNode::process (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
...
void ThresholdNode::setApplyOnADC (bool state)
{
...
void ThresholdNode::saveCustomChannelParametersToXml (XmlElement* channelInfo, int channelNumber, bool isEventChannel)
{
    if (! isEventChannel
        && channelNumber > -1
...
void ThresholdNode::loadCustomChannelParametersFromXml (XmlElement* channelInfo, bool isEventChannel)
{
